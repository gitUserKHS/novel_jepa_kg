...
- **기획** — 두 가지. `💬 대화로 기획` 은 **말로 한다**: "비 오는 항구 도시의 잔잔한 미스터리, 주인공은
  ...
  제안하고, 대화로 다듬은 뒤 `이 설정으로 집필 시작` 을 누르면 작품이 만들어진다. `📝 직접 작성` 은
...
- **첫 장** — 집필 시작을 눌러도 바로 생성하지 않는다. 집필 화면의 **첫 장을 쓰기 전에** 판에서 할루시네이션 강도·
  이번 턴 분량·첫 장면 요청(기획 대화의 첫 말이 미리 채워진다)을 정하고 `✍️ 첫 장 쓰기` 를 눌러야 첫 장이 큐에
  들어간다. 요청 칸을 비우면 이야기 지도의 첫 대목부터 시작하고, 여기서 정한 강도·분량이 다음 턴의 기본값이 된다.
