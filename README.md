...
- **자동 이어쓰기** — 턴마다 다음 전개를 사람이 적지 않아도 된다. 작품 화면의
  `🤖 자동 이어쓰기 켜기` 를 누르면 워커가 턴을 마칠 때마다 이야기 지도와 요약
  메모리를 보고 다음 전개를 한두 문장으로 정해(그 문장이 🤖 말풍선에 남는다)
  같은 창의성·분량으로 다음 턴을 넣는다. 완결되거나 연속
  `consumer.auto_continue_max_turns`(기본 20)턴이 되면 멈추고, `끄기` 를 누르면
  지금 쓰는 턴까지만 쓴다. 켜진 채로 직접 전개를 적어 끼어들어도 되고, `🤖 다음
  한 턴만 AI 에게` 는 한 번만 맡긴다.
- **쓰던 장이 사라졌다면** — 생성기의 게이트가 그 장의 초안을 불합격시킨 것이다
  (본문 990자 미만 · 마지막 문장 미완 · 직전 장 베끼기 · 같은 구절 6회 이상 반복).
  반복만 문제면 되풀이된 문장·문단을 걷어내고 그대로 채택하고(`✂️`, 전개 유지,
  재생성 없음), 그래도 안 되면 한 번 다시 쓴다(`🔁`). 화면에는 사유·버려진 초안
  ("버려진 초안 보기")·채택 결정이 남고, 끝난 턴 아래에도 기록된다. 버려진 초안은
  저장되지 않으며 다시 쓸 때 프롬프트·메모리·다음 장 문맥 어디에도 들어가지 않는다
  (`tests/test_longform_retry_isolation.py`). 재생성은 `generation.enable_stability_retry`
  로 끌 수 있다.
