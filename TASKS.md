...
- [x] 집필 시작이 바로 생성하지 않도록 (2026-09-14). `_start_story` 는 작품만 만들고, 집필 화면의 '첫 장을 쓰기 전에'
      판(`_first_turn_panel`, main 에서 그림 — 1초 fragment 안에서는 글 칸이 버티지 못함)에서 강도·분량·첫 장면 요청을
      정해 `✍️ 첫 장 쓰기` 를 눌러야 첫 턴이 큐에 들어간다. 기획 대화의 첫 말은 요청 칸에 미리 채우고, 판에서 정한 값은
      fragment 의 슬라이더·분량 선택에 물려준다. 턴도 원고도 없는 작품(초기화·대화 전체 삭제 뒤 포함)이면 판이 다시 나온다.
